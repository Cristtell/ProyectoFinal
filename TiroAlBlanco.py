...
# Opcional: separa la cámara un poco del suelo
camera.position = (0, 2, -20)

# ---------- INTERFAZ ----------
cursor = Entity(model='quad', parent=camera.ui,
                color=color.black, scale=.02)
score      = 0
time_limit = 60          # segundos
time_left  = time_limit

txt_score = Text(text=f'Puntos: {score}',
                 position=(-.85,.45), scale=2,
                 origin=(0,0), color=color.black)

txt_time  = Text(text=f'Tiempo: {int(time_left)}',
                 position=( .65,.45), scale=2,
                 origin=(0,0), color=color.black)

# ---------- LÓGICA DE LOS BLANCOS ----------
targets = []
colors  = [color.red, color.orange, color.yellow, color.azure]

def spawn_target():
    """Crea un nuevo objetivo en una posición aleatoria frente a la cámara."""
    t = Entity(
        model='sphere',
        scale=uniform(1,2),                   # tamaño aleatorio
        color=choice(colors),
        position=(uniform(-12,12), uniform(1,7), 0),
        collider='sphere'
    )
    targets.append(t)

# Generamos algunos blancos iniciales
for _ in range(7):
    spawn_target()
...
def input(key):
    global score
    if key == 'left mouse down' and time_left > 0:
        hit = mouse.hovered_entity
        # ¿Le dimos a un target?
        if hit in targets:
            destroy(hit)
            targets.remove(hit)
            score += 1
            txt_score.text = f'Puntos: {score}'
            spawn_target()        # mantener la cantidad de blancos
...
def update():
    ...
    if time_left > 0:
        time_left -= time.dt
        txt_time.text = f'Tiempo: {int(time_left)}'
    else:
        # Fin del juego (solo se ejecuta una vez)
        if cursor.enabled:        # comprobar para no repetir
            cursor.enabled = False
            for t in targets:
                t.collider = None
            game_over = Text(
                text=f'¡Tiempo agotado!\nPuntaje final: {score}',
                origin=(0,0), scale=3, background=True,
