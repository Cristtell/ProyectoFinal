...
LEVEL_CONFIG = {
    1: {'targets': 10, 'speed': (10, 15), 'scale': 1.8, 'accuracy_goal': 50},
    2: {'targets': 10, 'speed': (15, 22), 'scale': 1.5, 'accuracy_goal': 60},
    3: {'targets': 10, 'speed': (20, 28), 'scale': 1.2, 'accuracy_goal': 75}
}
...
class TargetSphere(Entity):
    ...
    def hit(self):
        ...
        effect = Entity(model='sphere', color=color.white, scale=self.scale*1.5, position=self.world_position, shadow=False)
...
def start_level(level):
    global hits, points, shots_fired, game_active, current_level, targets_spawned, last_shot_time
    current_level = level
...
app = Ursina(title='AIM PRESICION DDC', borderless=False)
...
start_sound = Audio('assets/sounds/fondo.mp3', loop=False, autoplay=False, volume=0.5)
...
# --- Arma y Mira ---
pistol = Entity(parent=camera, position=(0.4, -0.4, 1))
pistol.body = Entity(parent=pistol, model='cube', scale=(0.12, 0.2, 0.5), color=color.black)
pistol.handle = Entity(parent=pistol, model='cube', scale=(0.12, 0.3, 0.2), position=(0, -0.15, -0.1), color=color.black)
pistol.slide = Entity(parent=pistol, model='cube', scale=(0.1, 0.15, 0.45), position=(0, 0.08, 0), color=color.gray)

rifle = Entity(parent=camera, position=(0.6, -0.5, 1.5), rotation=(0,0,0))
rifle.body = Entity(parent=rifle, model='cube', scale=(0.1, 0.1, 1.0), color=color.dark_gray)
rifle.barrel = Entity(parent=rifle, model='cube', scale=(0.05, 0.05, 0.6), position=(0, 0, 0.5), color=color.gray)
rifle.scope = Entity(parent=rifle, model='cube', scale=(0.04, 0.04, 0.2), position=(0, 0.05, 0.2), color=color.black)
rifle.handle = Entity(parent=rifle, model='cube', scale=(0.08, 0.2, 0.1), position=(0, -0.1, -0.4), color=color.black)
rifle.disable()

shotgun = Entity(parent=camera, position=(0.5, -0.6, 1.2), rotation=(0,0,0))
shotgun.body = Entity(parent=shotgun, model='cube', scale=(0.15, 0.15, 0.8), color=color.dark_gray)
shotgun.barrel = Entity(parent=shotgun, model='cube', scale=(0.08, 0.08, 0.5), position=(0, 0, 0.4), color=color.gray)
shotgun.pump = Entity(parent=shotgun, model='cube', scale=(0.1, 0.08, 0.2), position=(0, -0.05, 0.2), color=color.black)
shotgun.handle = Entity(parent=shotgun, model='cube', scale=(0.15, 0.25, 0.15), position=(0, -0.15, -0.3), color=color.black)
shotgun.disable()
